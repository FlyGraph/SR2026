...
class DiReP(BaseModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.schedulers.append(
            GradualWarmupScheduler(self.optG, multiplier=1, total_epoch=20)
        )
    ...
    def save_current_results(self):
        ...
        for idx in range(self.batch_size):
            ...
            if self.mean > 1 and hasattr(self, "outputs"):
                for output_index in range(len(self.outputs)):
                    ret_path.append(
                        "Out_round{}_{}".format(output_index, self.path[idx])
                    )
                    ret_result.append(
                        self.outputs[output_index][idx].detach().float().cpu()
                    )
    ...
    def model_test(self, sample_num):
        if self.opt["distributed"]:
            output, self.visuals = self.netG.module.restoration(
                self.cond_image, sample_num=sample_num
            )
        else:
