...
class DiReP(BaseModel):
    ...
    def get_current_visuals(self, phase="train"):
        dict = {
            ...
            "cond_image": (
                self.cond_image.detach()[:, 0, :, :].float().cpu().unsqueeze(dim=1) + 1
            )
            / 2,
        }
        if phase != "train":
    ...
    def val_step(self):
        ...
        with torch.no_grad():
            for val_data in tqdm.tqdm(self.val_loader):
                ...
                if self.opt["distributed"]:
                    self.output, self.visuals, self.gt_image = (
                        self.netG.module.validation(
                            self.cond_image,
                            y_0=self.gt_image,
                    ...
                    self.output, self.visuals, self.gt_image = self.netG.validation(
                        self.cond_image, y_0=self.gt_image, sample_num=self.sample_num
                    )
    ...
    def test(self):
        ...
        self.logger.info(f"Start testing loop. Loader length: {len(self.phase_loader)}")
        with torch.no_grad():
            for i, phase_data in enumerate(self.phase_loader):
                self.logger.info(f"Processing batch {i}/{len(self.phase_loader)}")
                self.set_input(phase_data)
                self.outputs = []
                for i in range(self.mean):
                    if self.opt["distributed"]:
                        output, self.visuals = self.netG.module.restoration(
                            self.cond_image,
                            sample_num=self.sample_num,
                            y_0=self.gt_image,
                            path=self.path,
                        ...
                        output, self.visuals = self.netG.restoration(
                            self.cond_image,
                            sample_num=self.sample_num,
                            y_0=self.gt_image,
                            path=self.path,
                ...
                for met in self.metrics:
                    ...
                    self.writer.add_scalar(key, value)
                self.writer.save_images(
                    self.save_current_results(), norm=self.opt["norm"]
                )
        ...
        csv_path = os.path.join(self.opt["path"]["results"], "test_metrics.csv")
        df = pd.DataFrame([test_log])
        if os.path.exists(csv_path):
            df.to_csv(csv_path, mode="a", header=False, index=False)
        else:
            df.to_csv(csv_path, index=False)
        self.logger.info("Saved test metrics to {}".format(csv_path))
