...
class EMDiffusenDataset(data.Dataset):  
    def __init__(
        self,
        data_root,
        data_len=-1,
        ...
        self.tfs = transforms.Compose(
            [
                transforms.Resize(
                    (image_size[0], image_size[1]), interpolation=interp_mode
                ),
                transforms.ToTensor(),
            ...
            for noise_level in os.listdir(cell_path):
                for img in sorted(os.listdir(os.path.join(cell_path, noise_level))):
                    if "tif" in img:
                        ...
                        gt_paths.append(
                            os.path.join(cell_path, noise_level, img).replace(
                                "wf", "gt"
                            )
                        )
...
class vEMDiffuseTrainingDatasetPatches(
    ...
    def __getitem__(self, index):
        ...
        lower_bound = (
            self.z_times // 2 if self.z_times % 2 == 0 else self.z_times // 2 + 1
        )
        ...
        img_up = self.loader(
            os.path.join(
                os.path.dirname(gt_file_name), str(file_index - upper_bound) + ".tif"
            )
        )
        img_below = self.loader(
            os.path.join(
                os.path.dirname(gt_file_name), str(file_index + lower_bound) + ".tif"
            )
        )
        gts = []
        for i in range(file_index - upper_bound + 1, file_index + lower_bound):
            gts.append(
                self.loader(
                    os.path.join(os.path.dirname(gt_file_name), str(i) + ".tif")
                )
            )
...
class vEMDiffuseTrainingDatasetVolume(
    data.Dataset
):  
    def __init__(
        self,
        ...
        self.method = method
        self.percent = percent
        self.augment_blur = augment_blur if augment_blur is not None else True
        self.augment_flip = augment_flip if augment_flip is not None else True
        self.augment_rotate = augment_rotate if augment_rotate is not None else True
        self.tfs = transforms.Compose(
            [
                transforms.ToTensor(),
                transforms.Normalize(mean=[0.5], std=[0.5]),
        ...
        self.height, self.width = imread(
            os.path.join(data_root, str(self.depth) + ".tif")
        ).shape
        self.loader = loader
        self.norm = norm
        self.image_size = image_size
    ...
    def __getitem__(self, index):
        ...
        img_up = Image.fromarray(
            imread(os.path.join(self.data_root, str(z_index - upper_bound) + ".tif"))[
                y_index : y_index + self.image_size[1],
                x_index : x_index + self.image_size[0],
            ]
        )
        img_below = Image.fromarray(
            imread(os.path.join(self.data_root, str(z_index + lower_bound) + ".tif"))[
                y_index : y_index + self.image_size[1],
                x_index : x_index + self.image_size[0],
            ]
        )
        ...
        for i in range(z_index - upper_bound + 1, z_index + lower_bound):
            gts.append(
                Image.fromarray(
                    imread(os.path.join(self.data_root, str(i) + ".tif"))[
                        y_index : y_index + self.image_size[1],
                        x_index : x_index + self.image_size[0],
                    ]
                )
...
class vEMDiffuseTestIsotropic(data.Dataset):
    def __init__(
        self,
        data_root,
        data_len=-1,
    ...
    def __getitem__(self, index):
        ...
        if self.is_flat:
            ...
            img_up = self.loader(
                os.path.join(
                    self.data_root,
                    str(file_index - upper_bound),
                    gt_file_name.split("/")[-1],
                )
            )
            img_below = self.loader(
                os.path.join(
                    self.data_root,
                    str(file_index + lower_bound),
                    gt_file_name.split(os.sep)[-1],
                )
            )
            gts = []
            for i in range(file_index - upper_bound + 1, file_index + lower_bound):
                gts.append(
                    self.loader(
                        os.path.join(
                            self.data_root, str(i), gt_file_name.split(os.sep)[-1]
                        )
                    )
                )
    ...
    def read_dataset(self, data_root):
        ...
        if max_file_num > 0 and max_folder_num == 0:
            ...
            for i in range(
                upper_bound, z_depth - lower_bound, self.z_times
            ):  
...
class vEMDiffuseTestAnIsotropic(data.Dataset):
    def __init__(
        self,
        data_root,
        data_len=-1,
