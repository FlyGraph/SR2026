...
class BaseModel:
    ...
    def train(self):
        while (
            ...
            if (
                self.epoch % self.opt["train"]["save_checkpoint_epoch"] == 0
                and self.epoch > 500
            ):
                self.logger.info(
                    "Saving the self at the end of epoch {:.0f}".format(self.epoch)
    ...
    def _prune_old_checkpoints(self):
        if self.opt["global_rank"] != 0:
            return
        keep_last_n = self.opt["train"].get("keep_checkpoint_max", 5)
        ...
        for f in os.listdir(ckpt_dir):
            ...
            if m:
                ep = int(m.group(1))
                epoch_to_files.setdefault(ep, []).append(f)
        if len(epoch_to_files) <= keep_last_n:
            return
        sorted_epochs = sorted(epoch_to_files.keys(), reverse=True)
        to_remove_epochs = sorted_epochs[keep_last_n:]
        for ep in to_remove_epochs:
            for f in epoch_to_files[ep]:
                path = os.path.join(ckpt_dir, f)
                try:
                    os.remove(path)
                    self.logger.info("Removed old checkpoint: {}".format(path))
                except OSError as e:
                    self.logger.warning("Failed to remove {}: {}".format(path, e))
